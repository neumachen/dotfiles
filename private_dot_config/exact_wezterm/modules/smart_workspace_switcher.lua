...
local run_child_process = function(cmd)
  ...
  if is_windows then process_args = { 'cmd', '/c', cmd } end
  local success, stdout, stderr = wezterm.run_child_process(process_args)

  if not success then wezterm.log_error("Child process '" .. cmd .. "' failed with stderr: '" .. stderr .. "'") end
  return stdout
end
...
function M.switch_to_prev_workspace()
  return wezterm.action_callback(function(window, pane)
    ...
    if current_workspace == previous_workspace and previous_workspace ~= nil then return end
