---
name: shiki-using-git-worktrees
...
**Announce at start:** "I'm using the shiki-using-git-worktrees skill to set up an isolated workspace."
...
2. ~/.config/shiki/worktrees/<project-name>/ (global location)
...
Per Shiki rule of fixing broken things immediately:
...
### For Global Directory (~/.config/shiki/worktrees)
...
case $LOCATION in
  .worktrees|worktrees)
    ...
    ;;
  ~/.config/shiki/worktrees/*)
    path="~/.config/shiki/worktrees/$project/$BRANCH_NAME"
    ;;
esac
...
```
You: I'm using the shiki-using-git-worktrees skill to set up an isolated workspace.
...
**Pairs with:**
- **shiki-finishing-a-development-branch** - Cleanup after work complete (when using manual worktrees)
