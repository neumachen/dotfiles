...
local function clear_transient_keymaps()
  if not M.config.transient_keymaps.enable then return end
  if state.source_buf and vim.api.nvim_buf_is_valid(state.source_buf) then
    local keys = {
...
local function dive_into_block(node)
  ...
  while node do
    if node:type() == 'block' or node:type() == 'statement_block' then
      ...
      if not found_child then break end
    else
      break
    end
  end
...
local function get_sticky_node()
  if not state.sticky_node then return nil end
  local ok, sr, sc = pcall(function() return state.sticky_node:range() end)
  if not ok then
...
local function get_nav_node() return get_sticky_node() or get_master_node() end
...
M.ts_tree_display = function()
  ...
  if not node then
    ...
    if state.tree_win and not state.is_navigating then close_tree_win() end
    return
  end
  ...
  for i = 0, root_of_view:child_count() - 1 do
    ...
    if child and child:named() then table.insert(children, child) end
  end
  ...
  for i, child in ipairs(children) do
    ...
    if is_target then
      ...
      for j, grandchild in ipairs(grandchildren) do
        ...
        local g_marker = g_is_last and conf.icons.branch_end
          or conf.icons.branch_mid
  ...
  if state.tree_win and vim.api.nvim_win_is_valid(state.tree_win) then
    ...
    vim.api.nvim_create_autocmd(
      { 'CursorMoved', 'InsertEnter', 'BufLeave', 'BufWipeout' },
      {
        group = state.tree_grp,
        buffer = state.source_buf,
        callback = function(ev)
          if state.is_navigating then return end
          if ev.event == 'CursorMoved' and get_sticky_node() then return end
          close_tree_win()
        end,
      }
    )
  ...
  for i = 0, count - 1 do
    ...
    if child and child:named() then
      ...
      if r ~= root_start_row or c ~= root_start_col then
        ...
        local found =
          find_first_child_jump(child, root_start_row, root_start_col)
