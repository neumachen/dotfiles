...
if as.falsy(mein_wissen_path) then
  return {}
end
...
local function sync(path)
  return fmt("%s/%s", mein_wissen_path, path)
end
...
return {
  ...
  },
  {
    ...
    opts = {
        ...
        menu = {
          handler = function(data)
            local items = vim.tbl_filter(function(i)
              return i.key and i.label:lower() ~= "quit"
            end, data.items)
            ui.select(items, {
      ...
      org_default_notes_file = sync("orgfiles/notes.org"),
  ...
  },
  {
    ...
    config = function()
      require("telekasten").setup({
        ...
        dailies = sync("notes/daily"),
        weeklies = sync("notes/weekly"),
        templates = sync("notes/templates"),

        template_new_note = fmt("%s/notes/templates/new_note_tk.md", mein_wissen_path),
        template_new_daily = fmt("%s/notes/templates/daily_tk.md", mein_wissen_path),
        template_new_weekly = fmt("%s/notes/templates/weekly_tk.md", mein_wissen_path),
