...
function config.adjust_split_height(min_height, max_height)
...
local function is_list_open(list_type)
  return vim.iter(fn.getwininfo()):find(function(win) return not config.falsy(win[list_type]) end) ~= nil
end
...
function config.list.qf.toggle()
...
function config.list.loc.toggle()
...
function config.list.qf.delete(buf)
...
---@return string
function config.truncate(str, max_len)
  assert(str and max_len, 'string and max_len must be provided')
  return api.nvim_strwidth(str) > max_len and str:sub(1, max_len) .. config.ui.icons.misc.ellipsis or str
end

---Determine if a value of any type is empty
---@param item any
---@return boolean?
function config.falsy(item)
...
function config.pcall(msg, func, ...)
  local args = { ... }
...
function config.nightly() return vim.version().minor >= LATEST_NIGHTLY_MINOR end
...
function config.ftplugin_conf(configs)
  if type(configs) ~= 'table' then return end
  for name, callback in pairs(configs) do
    local ok, plugin = config.pcall(require, name)
...
function config.filetype_settings(map)
    ...
    return {
      ...
      command = function(args)
        vim.iter(settings):each(function(key, value)
          ...
          if key == 'plugins' then return config.ftplugin_conf(value) end
          if type(key) == 'function' then return config.pcall(key, args) end
          vim.iter(value):each(function(option, setting) vim[key][option] = setting end)
        end)
      end,
    }
  end)
  config.augroup('filetype-settings', unpack(commands:totable()))
end
...
--- Validate the keys passed to config.augroup are valid
---@param name string
...
---@param opts table?
function config.command(name, rhs, opts)
  opts = opts or {}
...
---@return boolean
function config.has(feature) return fn.has(feature) > 0 end
...
function config.reqidx(require_path)
  return setmetatable({}, {
...
function config.reqcall(require_path)
  return setmetatable({}, {
