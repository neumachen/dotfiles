...
local filetypes = config.p_table({
...
local filenames = config.p_table({
...
function config.ui.decorations.get(opts)
...
function config.ui.decorations.set_colorcolumn(bufnr, fn)
  local buf = vim.bo[bufnr]
  local decor = config.ui.decorations.get({ ft = buf.ft, bt = buf.bt, setting = 'colorcolumn' })
  if buf.ft == '' or buf.bt ~= '' or decor.ft == false or decor.bt == false then return end
  local ccol = decor.ft or decor.bt or ''
  local virtcolumn = not config.falsy(ccol) and ccol or '+1'
  if vim.is_callable(fn) then fn(virtcolumn) end
end

----------------------------------------------------------------------------------------------------
config.ui.current = { border = config.ui.border.line }
