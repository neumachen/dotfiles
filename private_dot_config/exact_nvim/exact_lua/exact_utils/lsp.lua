...
--   return {
--     -- NOTE: moved to tiny-code-action.nvim plugin
...
M.dprint_config_path = function() return get_config_path('dprint.json') end
...
M.deno_config_exist = function()
  local has_config = get_config_path('deno.json')
    or get_config_path('deno.jsonc')
  return has_config ~= nil
end

M.spectral_config_path = function() return get_config_path('.spectral.yaml') end
...
M.eslint_config_exists = function()
  ...
  for _, file in ipairs(config_files) do
    ...
    if vim.fn.filereadable(config_file) == 1 then return true end
  end
